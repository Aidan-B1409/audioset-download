...
class Downloader:
    ...
    def download_file(
        ...
        newpath = os.path.join(self.root_path, first_display_label, ytid)
        if os.path.exists(newpath):
            return

        os.system(
            f'yt-dlp -x --audio-format {self.format} --audio-quality {self.quality} --output "{os.path.join(self.root_path, first_display_label, ytid)}_{start_seconds}-{end_seconds}.%(ext)s" --postprocessor-args "-ss {start_seconds} -to {end_seconds}" https://www.youtube.com/watch?v={ytid}'
        )
        ...
        if self.copy_and_replicate:
            ...
            for label in positive_labels.split(",")[1:]:
                display_label = self.machine_to_display_mapping[label]
                os.system(
                    f'cp "{os.path.join(self.root_path, first_display_label, ytid)}_{start_seconds}-{end_seconds}.wav" "{os.path.join(self.root_path, display_label, ytid)}_{start_seconds}-{end_seconds}.wav"'
                )
        return
